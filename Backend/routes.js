...
router.post('/login', async (req, res) => {
    ...
    try {
        const user = await getAsync(req.db, 'SELECT id, password_hash FROM Users WHERE email = ?', [email]);
...
router.post('/campaigns', authorize, async (req, res) => {
    const { name, goal_amount, target_payment_id, image_url, description } = req.body;
    const userId = req.userId; // Aus dem JWT-Token

    if (!name || !goal_amount || !target_payment_id) {
        return res.status(400).json({ message: 'Name, Zielbetrag und Bankkonto (Ziel-ID) sind erforderlich.' });
    }
...
router.post('/stripe-webhook', express.raw({ type: 'application/json' }), async (req, res) => {
    ...
    if (event.type === 'payment_intent.succeeded') {
        ...
        console.log(`Zahlung erfolgreich für Campaign ID: ${campaignId}, Betrag: ${amount}`);
        ...
        try {
            ...
            const insertSql = `
                ...
                VALUES (?, ?, ?, ?, 'succeeded')
            `;
            await runAsync(req.db, insertSql, [campaignId, amount, donorName, paymentIntent.id]);
...
router.post('/create-stripe-account', authorize, async (req, res) => {
    ...
    const user = await getAsync(req.db, 'SELECT stripe_account_id FROM Users WHERE id = ?', [req.userId]);
    ...
    if (!accountId) {
        ...
        await runAsync(req.db, 'UPDATE Users SET stripe_account_id = ? WHERE id = ?', [accountId, req.userId]);
    }
